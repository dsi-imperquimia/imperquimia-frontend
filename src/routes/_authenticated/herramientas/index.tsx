...
const getEstadoLabelWithIcon = (estado: EstadoHerramienta | "ACTIVOS") => {
  const map: Record<EstadoHerramienta | "ACTIVOS", { texto: string; icono: string }> = {
...
function RouteComponent() {
  ...
  const [filtroEstado, setFiltroEstado] = useState<EstadoHerramienta | "ACTIVOS">("ACTIVOS");
  const [filtroProyectoId, setFiltroProyectoId] = useState<number | undefined>(undefined);

  // Query para cargar las herramientas mapeando los filtros dinámicos
  const { data: herramientas, isPending, error, refetch, isRefetching } = useQuery<Herramienta[]>({
    queryKey: ["herramientas", { estado: filtroEstado, proyectoId: filtroProyectoId }],
    queryFn: () => {
      const estadoApi = filtroEstado === "ACTIVOS" ? undefined : filtroEstado;
      const proyectoApi = filtroEstado === EstadoHerramienta.EN_PROYECTO ? filtroProyectoId : undefined;
  ...
  useEffect(() => {
    if (!error) return;
    toast.danger("Error al cargar el inventario de herramientas. Inténtalo de nuevo.");
  }, [error]);

  const getEstadoBadge = (estado: EstadoHerramienta) => {
    const styles: Record<EstadoHerramienta, string> = {
      [EstadoHerramienta.DISPONIBLE]: "bg-emerald-50 text-emerald-700 border-emerald-200",
      [EstadoHerramienta.EN_PROYECTO]: "bg-blue-50 text-blue-700 border-blue-200",
      [EstadoHerramienta.MANTENIMIENTO]: "bg-amber-50 text-amber-700 border-amber-200",
    ...
    const textoVisual = estado === EstadoHerramienta.DANADA ? "DAÑADA" : estado.replace("_", " ");

    return (
      <span className={cn("px-2.5 py-0.5 text-xs font-semibold rounded-full border", styles[estado])}>
        {textoVisual}
      </span>
    );
  };
    ...
    <>
      ...
      <div className="flex flex-wrap gap-4 bg-gray-50 border border-gray-200 rounded-xl p-4 items-center mb-6">
        <div className="flex flex-col gap-1 min-w-[200px]">
          <span className="text-xs font-semibold text-gray-600">Filtrar por Estado</span>
          <Select
          ...
          >
            ...
            <Select.Popover>
              <ListBox>
                ...
                <ListBox.Item key="ACTIVOS" id="ACTIVOS" textValue="Ver todos (Excepto Desecho)">
        ...
        {filtroEstado === EstadoHerramienta.EN_PROYECTO && (
          <div className="flex flex-col gap-1 min-w-[250px]">
            ...
            <span className="text-xs font-semibold text-gray-600">Filtrar por Proyecto</span>
            <Select
              // CORRECCIÓN: Parseamos el id numérico a string usando selectedKey (singular) para que HeroUI lo pinte seleccionado
              selectedKey={filtroProyectoId !== undefined ? String(filtroProyectoId) : ""}
              placeholder="Todos los proyectos"
              variant="secondary"
              onSelectionChange={(val) => setFiltroProyectoId(val === "" || !val ? undefined : Number(val))}
            >
              <Select.Trigger><Select.Value /></Select.Trigger>
              <Select.Popover>
                <ListBox>
              ...
              <Table.Collection items={items}>
                {(herramienta) => (
                  <Table.Row key={herramienta.id}>
                    ...
                    <Table.Cell className="font-medium">{herramienta.nombre}</Table.Cell>
                    <Table.Cell>{herramienta.marca}</Table.Cell>
                    <Table.Cell className="text-gray-500">{herramienta.tipo}</Table.Cell>
                    <Table.Cell>{getEstadoBadge(herramienta.estado)}</Table.Cell>
                    <Table.Cell>
                      {herramienta.proyecto ? (
                        <span className="text-blue-700 font-semibold">🚧 {herramienta.proyecto.nombre}</span>
                      ) : (
                    ...
                    <Table.Cell>
                      <div className="inline-flex items-center gap-2 justify-end">
                        ...
                        {herramienta.estado !== EstadoHerramienta.DESECHO ? (
                          ...
                          <span className="text-xs text-gray-400 italic px-3">De baja</span>
                        )}
                      </div>
                    </Table.Cell>
                  </Table.Row>
